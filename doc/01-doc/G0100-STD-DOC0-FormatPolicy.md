...
DocFoundary 標準文書は **日本語を正本（`inLanguage: ["ja"]`）** とする。  
ただし、理解補助や国際的用語の明確化のため、必要に応じて **英語を補助的に併記してよい（SHOULD）**。

#### (1) 表記原則
* 英語が国際的に確立している技術用語（例: Front Matter, Markdown, YAML）は **英語表記を主とする（MUST）**。
* 日本語訳が自然または一般的な場合は日本語表記を主とし、必要に応じて括弧内に英語を併記して可（SHOULD）。
* 同一文書内では、各用語の表記形を統一し、混用してはならない（MUST NOT）。

#### (2) 見出しの扱い
* 見出しは原則として **日本語のみ** を用いる（MUST）。
* 英語の併記は、以下のいずれかに該当する場合のみ許可する（SHOULD）:
  1. 英語の方が国際的に認知された正式名称である場合  
     （例：`## 4.2 Front Matter構造（Front Matter Structure）`）
  2. 日本語訳のみでは意味が不明瞭または誤解を招くおそれがある場合  
  3. 外部仕様・他標準文書との参照整合性を保つ必要がある場合
